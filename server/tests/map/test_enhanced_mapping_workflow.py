...
from server.map.mapper import MappingEngine
from server.map.fingerprint import generate_fingerprint, generate_enhanced_fingerprint
from server.subscriptions import database as db_service
    ...
    async def mock_get_ingestion_mappings_by_structure(structure_fingerprint: str):
        ...
        for mapping in stored_mappings.values():
            if mapping.structure:
                from server.map.fingerprint import create_canonical_string
                import hashlib
