...
class E2ETestUtils:
    ...
    async def cleanup_test_data(self):
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                conn.execute(
                    text("DELETE FROM subscriptions WHERE subscriber_id = 'test-user'")
                )
                conn.execute(
                    text(
                        "DELETE FROM subscriptions WHERE description LIKE '%E2E_TEST%'"
                    )
                )
                conn.commit()
        except Exception as e:
    ...
    async def send_test_event(
        self, source: str = "github", payload: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    async def list_subscriptions(self, page: int = 1, size: int = 50) -> dict[str, Any]:
        ...
        response = await self.http_client.get(
            f"/subscriptions/?page={page}&size={size}"
        )
        response.raise_for_status()
        return response.json()

    async def update_subscription(
        self, subscription_id: int, updates: dict[str, Any]
    ) -> dict[str, Any]:
        """Update a subscription."""
        response = await self.http_client.put(
            f"/subscriptions/{subscription_id}", json=updates
        )
        response.raise_for_status()
        return response.json()
