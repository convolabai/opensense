...
async def test_llm_transformation_flow():
    ...
    with (
        ...
        mock_llm.transform_to_canonical = AsyncMock(
            return_value={
                "publisher": "github",
...
async def test_llm_unavailable_flow():
    ...
    with (
        ...
        raw_event = {
            ...
            "signature_valid": True,
            "headers": {},
            "payload": {"action": "created", "item": {"id": 789}},
        }
        ...
        assert (
            "No mapping available and LLM service unavailable" in failure_event["error"]
        )
