...
async def test_transform_to_canonical_success(mock_llm_service):
    ...
    mock_response.generations[0][
        0
    ].text = '{"publisher": "github", "resource": {"type": "pull_request", "id": 123}, "action": "created"}'
...
async def test_transform_to_canonical_missing_fields(mock_llm_service):
    ...
    mock_response.generations[0][
        0
    ].text = '{"publisher": "github", "action": "created"}'  # Missing resource
...
def test_validate_canonical_format_allows_slash_in_resource_id():
    ...
    canonical_data = {
        ...
        "resource": {
            "type": "pull_request",
            "id": "123/456",
        },  # Valid ID with slash (now allowed)
        "action": "created",
    }
