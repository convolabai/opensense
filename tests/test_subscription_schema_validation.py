...
def client():
    ...
    with (
        patch("langhook.ingest.nats.nats_producer") as mock_nats,
        patch("langhook.map.service.mapping_service") as mock_mapping,
        patch(
            "langhook.ingest.middleware.RateLimitMiddleware.is_rate_limited"
        ) as mock_rate_limit,
...
def test_subscription_creation_with_no_suitable_schema(client):
    ...
    with patch(
        "langhook.subscriptions.routes.llm_service.convert_to_pattern"
    ) as mock_convert:
        # Mock the LLM service to raise NoSuitableSchemaError
        mock_convert.side_effect = NoSuitableSchemaError(
            "No suitable schema found for description"
        )
        ...
        subscription_data = {
            ...
            "channel_config": {"url": "http://example.com/webhook", "method": "POST"},
        }
...
def test_subscription_update_with_no_suitable_schema(client):
    ...
    with (
        ...
        update_data = {"description": "Notify me about Discord messages"}
...
def test_subscription_creation_with_valid_schema(client):
    ...
    with (
        ...
        assert (
            subscription["pattern"] == "langhook.events.github.pull_request.*.updated"
        )
