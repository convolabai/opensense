...
class LLMSuggestionService:
    ...
    def __init__(self) -> None:
        ...
        if settings.openai_api_key:
            try:
                ...
                logger.error(
                    "Failed to initialize OpenAI LLM", error=str(e), exc_info=True
                )
                raise e
        else:
    ...
    async def transform_to_canonical(
...
Example canonical format:
{
  ...
  "action": "created"
    ...
    def _validate_canonical_format(
        self, canonical_data: dict[str, Any], source: str
    ) -> bool:
        ...
        missing_fields = [
            field for field in required_fields if field not in canonical_data
        ]
