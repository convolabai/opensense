...
class NATSEventProducer(BaseNATSProducer):
    ...
    async def send_canonical_event(self, canonical_data: dict[str, Any]) -> None:
        ...
        await self.publish_message(
            subject, canonical_data, headers=headers, log_success=True
        )
    ...
    async def send_raw_event(self, event: dict[str, Any]) -> None:
        ...
        await self.publish_message(subject, event, log_success=True)
    ...
    async def send_dlq(self, dlq_event: dict[str, Any]) -> None:
        ...
        try:
            await self.publish_message(subject, dlq_event, log_success=False)
            logger.debug(
                "Event sent to DLQ",
                subject=subject,
